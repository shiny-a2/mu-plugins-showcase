...
APPROVED_PATHS = frozenset(
    {
        ...
        "samples/README.md",
        "samples/php/admin-action-policy.php",
        "samples/php/bounded-cleanup-loop.php",
        "samples/php/request-classifier.php",
...
# This repository publishes deliberately rewritten sample code, so PHP under
# samples/ is expected. That is a narrow exemption from the embedded-source rule
# and nothing else: every other rule still applies to those files, and a sample
# outside samples/ is still refused. Pinned by a test.
SAMPLE_SOURCE_PREFIX = "samples/"
...
def content_failures(relative: str, data: bytes) -> List[str]:
    ...
    if relative != "scripts/check_public_safety.py":
        if not relative.startswith(SAMPLE_SOURCE_PREFIX) and EMBEDDED_SOURCE.search(text) is not None:
