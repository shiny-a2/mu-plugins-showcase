...
class PublicSafetyTests(unittest.TestCase):
    ...
    def test_the_samples_exemption_is_narrow(self) -> None:
        # This repository publishes rewritten sample code on purpose, so PHP
        # under samples/ is expected. That exemption must cover the
        # embedded-source rule and nothing else, or samples/ becomes the place
        # anything can be hidden.
        php = "<" + "?php echo 1;"

        self.assertNotIn(
            "embedded-source",
            CHECKER.content_failures("samples/php/example.php", php.encode("utf-8")),
            "sample source is allowed under samples/",
        )
        self.assertIn(
            "embedded-source",
            CHECKER.content_failures("docs/notes.md", php.encode("utf-8")),
            "the same content outside samples/ is still refused",
        )

        # Every other rule still applies inside samples/.
        for content, rule in (
            ("-----BEGIN " + "RSA PRIVATE KEY" + "-----\nMIIE\n", "private-key"),
            (".".join(["203", "0", "113", "7"]), "ip-address"),
            ("0" + "912" + "3456789", "mobile-number"),
            ("the " + "javaher" + "ian store", "client-or-infrastructure-identifier"),
            ("f" * 64, "long-hex-digest"),
        ):
            with self.subTest(rule=rule):
                self.assertIn(
                    rule,
                    CHECKER.content_failures("samples/php/example.php", content.encode("utf-8")),
                    f"{rule} is still refused inside samples/",
                )
